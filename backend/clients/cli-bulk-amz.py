...
def update_database():
    ...
    requests.get(f"{BASE_URL}/api/v1/amazon/orders/unshipped", params=params)

def display_unshipped_orders():
    # 获取未发货订单号。
    params = {"api_key_index": api_key_index, "up_to_date": True}
    resp = requests.get(f"{BASE_URL}/api/v1/amazon/orders/unshipped", params=params)
    orderIds = resp.json()['data']['orderNumbers']
    # 生成装箱单下载链接
    resp = requests.get(f"{BASE_URL}/api/v1/amazon/orders/sc-urls")
    data = resp.json()['data']
...
def sim_upload_amazon_pack_slips():
    ...
    resp = requests.post(endpoint, data=text)
...
def upload_amazon_pack_slips_via_file():
    ...
    resp = requests.post(endpoint, data=html_content.encode('utf-8'), headers=headers)
    key = resp.json()['data']['key']
...
def test_create_parcel_labels(orderIds):
    ...
    resp = requests.post(endpoint, json=orderIds)
    print(resp.url)
...
def test_get_manifest(batchId):
    ...
    resp = requests.get(f"{BASE_URL}/api/v1/pickpack/common/batch-ship-event", params={'batchId': batchId})
    data = resp.json()['data']
    print(resp.url)
    ...
    refs = ";".join(orderIds)
    resp = requests.get(f"{BASE_URL}/api/v1/pickpack/amazon/batch-pick", params={'refs': refs})
    print(resp.url)
    with open(f'{folder}/pick-slip.xlsx', 'wb') as f:
        f.write(resp.content)

    # Save pack slip as excel
    resp = requests.get(f"{BASE_URL}/api/v1/pickpack/amazon/batch-pack", params={'refs': refs})
    print(resp.url)
    with open(f'{folder}/pack-slip.xlsx', 'wb') as f:
        f.write(resp.content)

    # Save parcels as pdf
    resp = requests.get(f"{BASE_URL}/api/v1/carriers/gls/shipments/bulk-labels", params={'refs': refs})
    print(resp.url)
